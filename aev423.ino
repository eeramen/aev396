...
//#define pressuresbt   true
#define pressuresbt   false
//#define rumblebt      true
#define rumblebt      false
...
int samples = 0;
int analogMillis = 0;
int analogtimer = 900; //analog polling rate
...
pinMode(A5,INPUT);//5v
pinMode(35,OUTPUT);
//pinMode(PS2light1, OUTPUT);
//pinMode(PS2light2, OUTPUT);
digitalWrite(35,LOW);
//Serial.print("jfdgfshjkhljror");
  ...
  // TWBR = 12;  // 400 kbit/sec I2C speed
   ...
   error = ps2x.config_gamepad(PS2_CLK, PS2_CMD, PS2_SEL, PS2_DAT, pressures, rumble);
    if (error != 0){
      ...
      lcd.print("PS2 Error"); delay(5000);}
      else {lcd.print("PS2 Connected");}
   errorbt = ps2xbt.config_gamepad(PS2bt_CLK, PS2bt_CMD, PS2bt_SEL, PS2bt_DAT, pressuresbt, rumblebt);
    if (errorbt != 0){
      Serial.print("ps2bt Error");
      lcd.setCursor(0,1);
      lcd.print("PS2bt Error"); delay(5000);}
      else {lcd.setCursor(0,1);lcd.print("PS2bt Connected");}
...
void loop(){  
  ...
  vibrate = ps2x.Analog(PSAB_CROSS);  //this will set the large motor vibrate speed based on how hard you press the blue (X) button
      ...
      if (ps2x.Button(PSB_SELECT)){
       errorbt = ps2xbt.config_gamepad(PS2bt_CLK, PS2bt_CMD, PS2bt_SEL, PS2bt_DAT, pressuresbt, rumblebt);
       if (errorbt == 0){
        lcd.clear();lcd.print("PS2 BT connected");
       }
      }
     ps2xbt.read_gamepad(false,vibrate);
     vibrate = ps2xbt.Analog(PSAB_CROSS);  //this will set the large motor vibrate speed based on how hard you press the blue (X) button
     if(errorbt == 0){
     if (ps2xbt.Button(PSB_L1)||ps2xbt.Button(PSB_R1)){
      // find y-axis values
      lyy = ps2xbt.Analog(PSS_LY);
      ryy = ps2xbt.Analog(PSS_RY);
    Serial.print("lyy: ");
    Serial.print(lyy);
    Serial.print("ryy: ");
    Serial.println(ryy);
    ...
    if(ps2x.Button(PSB_CROSS)){ //|ps2xbt.Button(PSB_CROSS)){
        right1.writeMicroseconds(1500);
        left1.writeMicroseconds(1500);} 
   
   if (ps2x.Button(PSB_TRIANGLE)){//||ps2xbt.Button(PSB_TRIANGLE)){
      digitalWrite(35,HIGH);
      Serial.print("triangle");}
   if (!ps2x.Button(PSB_TRIANGLE)){
      digitalWrite(35,LOW);}
   if (ps2x.Button(PSB_SQUARE)){//||ps2xbt.Button(PSB_SQUARE)){
        digitalWrite(35,LOW);
        Serial.print("square");}
...
if (currentMillis - analogMillis > analogtimer){
  ...
  break;
    ...
    case 4:
      volt12 = analogRead(A4)*.012012012;
      Serial.print("12 rail: ");
      Serial.println(volt12);
      analogsignal = 5;
      break;
    case 5:
      volt5 = analogRead(A5)*.00945179;
      Serial.print("5 rail: ");
