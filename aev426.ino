...
//Servo right2;         // attached to piun 
Servo left1;          // attached to pin 
//Servo left2;          // attached to pin 
...
/*   errorbt = ps2xbt.config_gamepad(PS2bt_CLK, PS2bt_CMD, PS2bt_SEL, PS2bt_DAT, pressuresbt, rumblebt);
    if (errorbt != 0){
      ...
      lcd.setCursor(0,1);
      lcd.print("PS2bt Error"); 
      delay(5000);}
      else {
      lcd.setCursor(0,1);
      lcd.print("PS2bt Connected");
     Serial.print("PS2bt Connected");}
...
void loop(){  
  ...
  vibrate = ps2x.Analog(PSAB_CROSS);  //this will set the large motor vibrate speed based on how hard you press the blue (X) button
      ...
      if (ps2x.Button(PSB_SELECT)){
       errorbt = ps2xbt.config_gamepad(PS2bt_CLK, PS2bt_CMD, PS2bt_SEL, PS2bt_DAT, pressuresbt, rumblebt);
       if (errorbt == 0){
        Serial.print("PS2 BT probs connected, idfk");
...
if (currentMillis - analogMillis > analogtimer){
  ...
  switch (analogsignal){
    case 0:
      ...
      tempbat = (batRA.getAverage()*0.48828125*9/25) +32;
